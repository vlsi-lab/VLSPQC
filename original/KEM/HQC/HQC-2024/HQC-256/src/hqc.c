 ...
 */
void print_shake256incctx(shake256incctx *state) {
    printf("shake256incctx state:\n");
    printf("ctx: [");
    for (int i = 0; i < 25; i++) {
        printf("%lu, ", state->ctx[i]);
    }
    printf("%lu]\n", state->ctx[25]);
}
...
void hqc_pke_keygen(unsigned char* pk, unsigned char* sk, const uint8_t *sk_seed, const uint8_t *pk_seed) {
    ...
    //print_shake256incctx(&sk_seedexpander);
    
    // Compute public key
    vect_set_random(&pk_seedexpander, h);
    //print_shake256incctx(&pk_seedexpander);
    vect_mul(s, y, h);
