...
/* Contiguous layout for the PRF_addr SHAKE256 input: pub_seed || addr || sk_seed.
...
{
    prf_addr_input_t input;

    memcpy(input.pub_seed, ctx->pub_seed, SPX_N);
    memcpy(input.addr, addr, SPX_ADDR_BYTES);
    memcpy(input.sk_seed, ctx->sk_seed, SPX_N);

    shake256(out, SPX_N, (const uint8_t *)&input, sizeof(input));
}
