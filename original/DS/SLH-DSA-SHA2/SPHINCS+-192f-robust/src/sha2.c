...
#ifdef SPX_RISCV_SHA2_ACCEL
#include "sha256_rvcrypto.h"
#include "sha512_rvcrypto.h"
#undef Sigma0_32
#undef Sigma1_32
#undef sigma0_32
#undef sigma1_32
#define Sigma0_32(x) spx_sha256sum0((uint32_t)(x))
#define Sigma1_32(x) spx_sha256sum1((uint32_t)(x))
#define sigma0_32(x) spx_sha256sig0((uint32_t)(x))
#define sigma1_32(x) spx_sha256sig1((uint32_t)(x))
#endif
...
#undef sigma0_64
#undef sigma1_64
#define Sigma0_64(x) spx_sha512sum0((uint64_t)(x))
#define Sigma1_64(x) spx_sha512sum1((uint64_t)(x))
#define sigma0_64(x) spx_sha512sig0((uint64_t)(x))
#define sigma1_64(x) spx_sha512sig1((uint64_t)(x))
#endif
...
#if defined(SPX_CW305_COMPACT_SHA512)
...
static int crypto_hashblocks_sha512(unsigned char *statebytes,const unsigned char *in,unsigned long long inlen)
{
  ...
  while (inlen >= 128) {
    ...
    for (size_t i = 0; i < 80; i++) {
      ...
      if (i >= 16) {
        w[i & 15] += sigma1_64(w[(i + 14) & 15]) + w[(i + 9) & 15] +
                     sigma0_64(w[(i + 1) & 15]);
      }
      wi = w[i & 15];

      T1 = h + Sigma1_64(e) + Ch(e, f, g) + K512[i] + wi;
      T2 = Sigma0_64(a) + Maj(a, b, c);
      h = g;
      g = f;
      f = e;
  ...
  for (size_t i = 0; i < 8; i++) {
    store_bigendian_64(statebytes + 8 * i, state[i]);
  }
