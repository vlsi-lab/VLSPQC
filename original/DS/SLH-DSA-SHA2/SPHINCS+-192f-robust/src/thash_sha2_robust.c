           ...
           const spx_ctx *ctx, uint32_t addr[8])
{
    ...
    uint8_t *masked = buf + SPX_N + SPX_SHA256_ADDR_BYTES;
    uint8_t sha2_state[40];
    unsigned int i;

    memcpy(buf, ctx->pub_seed, SPX_N);
    memcpy(buf + SPX_N, addr, SPX_SHA256_ADDR_BYTES);
    mgf1_256(masked, inblocks * SPX_N, buf, SPX_N + SPX_SHA256_ADDR_BYTES);
           ...
           const spx_ctx *ctx, uint32_t addr[8])
{
    ...
    memcpy(buf, ctx->pub_seed, SPX_N);
    memcpy(buf + SPX_N, addr, SPX_SHA256_ADDR_BYTES);
    mgf1_512(masked, inblocks * SPX_N, buf, SPX_N + SPX_SHA256_ADDR_BYTES);
