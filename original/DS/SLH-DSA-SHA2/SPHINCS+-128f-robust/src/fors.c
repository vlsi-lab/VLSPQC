               ...
               const uint32_t fors_addr[8])
{
    ...
    SPX_PROGRESS_MSG("fors_sign start");
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        if (i == 0 || ((i + 1) % 4) == 0 || (i + 1) == SPX_FORS_TREES) {
            SPX_PROGRESS_U32_PAIR("fors_sign tree ", i + 1, "/", SPX_FORS_TREES, "");
        }
    ...
    SPX_PROGRESS_MSG("fors_sign done");
}
                      ...
                      const uint32_t fors_addr[8])
{
    ...
    SPX_PROGRESS_MSG("fors_pk_from_sig start");
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        if (i == 0 || ((i + 1) % 4) == 0 || (i + 1) == SPX_FORS_TREES) {
            SPX_PROGRESS_U32_PAIR("fors_pk_from_sig tree ", i + 1, "/", SPX_FORS_TREES, "");
        }
    ...
    /* Hash horizontally across all tree roots to derive the public key. */
    thash(pk, roots, SPX_FORS_TREES, ctx, fors_pk_addr);
    SPX_PROGRESS_MSG("fors_pk_from_sig done");
}
