                ...
                void *info)
{
    ...
    uint32_t progress_step = 1;

    if (tree_height > 4) {
        progress_step = 1U << (tree_height - 2);
    } else if (tree_height > 2) {
        progress_step = 1U << (tree_height - 2);
    }

    SPX_PROGRESS_U32("treehashx1 height ", tree_height, " start");
    for (idx = 0;; idx++) {
        ...
        if ((idx % progress_step) == 0U) {
            SPX_PROGRESS_U32_PAIR("treehashx1 leaf ", idx + 1, "/", max_idx + 1, "");
        }
        ...
        for (h=0;; h++, internal_idx >>= 1, internal_leaf >>= 1) {
            ...
            if (h == tree_height) {
                ...
                SPX_PROGRESS_U32("treehashx1 height ", tree_height, " done");
                return;
            }
