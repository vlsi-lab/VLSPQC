...
static inline  word_t MXOR(word_t a, word_t b, int ns) {
...
static inline  word_t MXORAND(word_t c, word_t a, word_t b, int ns) {
  b.s[0].w[0] = ~b.s[0].w[0];
  b.s[0].w[1] = ~b.s[0].w[1];
...
static inline  word_t MRND(int ns) {
  word_t w;
...
static inline  word_t MMIX(word_t w, int ns) {
...
static inline  word_t MREDUCE(word_t w, int nsi, int nso) {
...
static inline  word_t MEXPAND(word_t w, int nsi, int nso) {
...
static inline  word_t MZERO(int ns) {
  word_t w;
  if (ns == 1) {
    MOVI(&w.s[0].w[0], 0);
    MOVI(&w.s[0].w[1], 0);
  ...
  if (ns >= 3) {
	...
	w.s[2].w[0]=ROR32(w.s[2].w[0], 13);
    w.s[2].w[1]=ROR32(w.s[2].w[1], 13);
  ...
  if (ns >= 4) {
	...
	w.s[3].w[0]=ROR32(w.s[3].w[0], 29);
    w.s[3].w[1]=ROR32(w.s[3].w[1], 29);
...
static inline  word_t MMASK(int n, int ns) {
...
static inline  word_t MREFRESH(word_t w, int ns) {
  word_t r = MZERO(ns);
  return MXOR(w, r, ns);
}

static inline  int MNOTZERO(word_t a, word_t b, int ns) {
  word_t c = MZERO(ns);
  /* note: OR(a,b) = ~BIC(~a,b) */
  a.s[0].w[0] = ~a.s[0].w[0];
  a.s[0].w[1] = ~a.s[0].w[1];
...
static inline share_t LOADSHARE(uint32_t* data, int ns) {
  share_t s;
  uint32_t lo, hi;
  LDR(&lo, data, 0);
  LDR(&hi, data, ns);
...
static inline  void STORESHARE(uint32_t* data, share_t s, int ns) {
  uint32_t lo, hi;
#if !ASCON_EXTERN_BI
...
static inline  word_t MLOAD(uint32_t* data, int ns) {
  word_t w = {0};
...
static inline  void MSTORE(uint32_t* data, word_t w, int ns) {
